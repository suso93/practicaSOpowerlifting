...
//al recibir SIGUSR1:
	...
	if(haySitioEnCampeonato()!=-1) {
		...
		//creo hilo de atleta:
		//pthread_create();
		AccionesAtleta(posicion);//ver qué le pasamos, creo que habria que pasarle id atleta y id tarima
	} else { 
		printf("Ya están inscritos y participando 10 atletas, de momento no puedes participar\n");
	}
}
void AccionesAtleta (int posicion){ //creo que hay que pasarle o la posicion o el id
	//guardar en log:
